...
export default function PlanSelectionPage() {
  ...
  const handleContinue = () => {
    // Store selected plan in sessionStorage for account creation
    ...
    <div className="min-h-screen px-4 py-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-ink-text-muted mb-8">
          ...
          {tiers.map((tier) => {
            ...
            return (
              <button
                ...
                className={`relative text-left rounded-3xl border-2 transition-all duration-300 ${
                  isSelected
                    ? tier.popular
                      ? 'border-purple-500 shadow-glow-lg scale-[1.02]'
                      : 'border-ink-accent shadow-glow scale-[1.02]'
                      ...
                      <>
                        ...
                        {billingCycle === 'yearly' && (
                          <div className="text-sm text-green-400 mt-1">
                            ${tier.yearlyPrice}/year • Save ${(tier.price * 12 - tier.yearlyPrice).toFixed(0)}
                          </div>
                        )}
                      </>
                    )}
                    ...
                    {tier.displayFeatures.map((feature, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm">
                        <CheckIcon className={`w-5 h-5 shrink-0 mt-0.5 ${
                          tier.popular ? 'text-purple-400' : 'text-cyan-400'
                        }`} />
