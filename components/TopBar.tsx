    ...
    <header className="relative z-30 border-b border-white/5 bg-white/[0.02] backdrop-blur-lg">
      ...
      <div className="relative mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-5 sm:px-8">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={user ? '/map' : '/login'}
            className="hidden rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.4em] text-ink-text-muted transition hover:border-ink-accent/40 hover:text-white sm:inline-flex"
          >
            {user ? 'Explore' : 'Log In'}
          </Link>
          {user ? (
            <>
              ...
              <Link
                href={pathname.startsWith('/chat') ? '/chat' : '/map'}
